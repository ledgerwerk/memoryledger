...
@dataclass(frozen=True)
class Memory:
    """Durable memory metadata; Markdown content is stored separately."""
...
@dataclass(frozen=True)
class RenderConfig:
    """Deterministic output and inclusion settings."""
...
@dataclass(frozen=True)
class Template:
    """A reusable memory definition loaded from global configuration."""
...
class TemplatePolicy:
    """Project policy controlling which templates may be applied."""
...
@dataclass(frozen=True)
class GlobalConfig:
    """User-level template configuration and its source path."""
...
@dataclass(frozen=True)
class Config:
    """Compatibility configuration combining project settings and paths."""
...
@dataclass(frozen=True)
class Workspace:
    """Resolved canonical or legacy workspace and its tool paths."""
...
class StorageDiscovery:
    """Read-only report describing discovered project storage."""
...
@dataclass(frozen=True)
class DerivedState:
    """Ledger version and next durable identifiers derived from storage."""
...
class LegacyInventory:
    """Inventory of files and counters in a legacy storage source."""
