...
Use `memoryledger memory create` for candidates, `memoryledger review accept` for explicit approval, then `memoryledger finalize --accept-all --reason "..." --export` when the user approved the candidates. Otherwise run `memoryledger render` and `memoryledger export` separately.
...
For legacy projects, use `memoryledger migrate storage-v2 --plan` before `--apply --backup`. Use `memoryledger migrate linked-docs-dir --from docs/agents --to agent_docs --plan` before `--apply`; it moves only generated files with the marker.
