...
def test_review_accept_all_accepts_only_candidates_and_reports_json(runner, work) -> None:
    invoke_ok(runner, ["init"])
    for title in ["One", "Two", "Three"]:
        invoke_ok(runner, ["memory", "create", "--kind", "rule", "--title", title, "--text", "Always test."])
    invoke_ok(runner, ["review", "reject", "memory-0002", "--reason", "No."])
    data = json.loads(invoke_ok(runner, ["review", "accept", "--all", "--reason", "Yes.", "--json"]).output)
...
def test_review_reject_all_requires_reason(runner, work) -> None:
    invoke_ok(runner, ["init"])
    invoke_ok(runner, ["memory", "create", "--kind", "rule", "--title", "Rule", "--text", "Always test."])
    result = runner.invoke(__import__("memoryledger.cli", fromlist=["app"]).app, ["review", "reject", "--all"])
